...
export function SubjectCards({ subjects, onSelect }: Props) {
  return (
    <div className="min-h-screen bg-bg px-12 pt-10">
      <div className="text-accent text-2xl font-bold mb-8 tracking-tight">Learn</div>
      <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))' }}>
        {subjects.map(s => {
          const pct = s.totalTopics > 0 ? Math.round((s.masteredTopics / s.totalTopics) * 100) : 0
          return (
            <button
              key={s.id}
            ...
            >
              ...
              <div className="flex gap-3 text-sm">
                <span className="text-muted">{s.masteredTopics}/{s.totalTopics} mastered</span>
                {s.dueToday > 0 && (
