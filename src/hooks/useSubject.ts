...
export function useTopics(subjectId: string | null) {
  ...
  useEffect(() => {
    if (!subjectId) return
    setLoading(true)
    rpc<Topic[]>('query_topics', { p_subject_id: subjectId, p_limit: 500 })
...
export function useTouches(subjectId: string | null) {
  ...
  useEffect(() => {
    if (!subjectId) return
    setLoading(true)
    rpc<Touch[]>('query_touches', {
      p_subject_id: subjectId,
      p_sort_field: 'createdAt',
      p_sort_dir: 'desc',
      p_limit: 1000,
    })
      .then(data => setTouches(data ?? []))
      .finally(() => setLoading(false))
  }, [subjectId])
...
export function useTopic(topicId: string | null) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    rpc<Topic & { history: Touch[] }>('get_topic', { p_topic_id: topicId })
